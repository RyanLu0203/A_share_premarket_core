...
from ashare_premarket.providers.registry import (
    REGISTRY_FIELDS,
...
def test_provider_registry_contains_required_policies() -> None:
    ...
    assert {"baostock", "akshare", "ifind", "local_import"}.issubset(providers)
    assert list(rows[0]) == REGISTRY_FIELDS
    assert providers["baostock"]["current_role"].startswith("committed_provider02b")
    assert providers["akshare"]["network_opt_in_policy"].startswith(
        "network_disabled_by_default"
    )
    assert "ASHARE_ALLOW_IFIND=1" in providers["ifind"]["network_opt_in_policy"]
    assert (
        providers["ifind"]["raw_data_commit_policy"]
        == "paid_raw_payloads_and_tokens_never_committed"
    )
    assert all(row["raw_data_commit_policy"] != "" for row in rows)
    assert all(
        row["forbidden_production_write_policy"] == "production_db_writes_forbidden"
        for row in rows
    )
