...
export function WatchlistPage({seed, stocks, selectedSymbol}: {seed: string[]; stocks: Stock[]; selectedSymbol?: string}) {
  ...
  const rows = watchlistRows.filter((row) => (bandFilter === "ALL" || row.band_status === bandFilter) && (abstentionFilter === "ALL" || abstentionState(row.abstain) === abstentionFilter));
  const columns: DenseColumn<Stock>[] = [
    ...
    {key: "abstain", label: "Abstain", render: (row) => <StatusBadge state={abstentionState(row.abstain)} />},
  ...
  return <div className="page-stack">
    ...
    <div className="filter-bar"><label><span>Band status</span><select aria-label="Watchlist band filter" value={bandFilter} onChange={(event) => setBandFilter(event.target.value)}><option value="ALL">All bands</option>{Array.from(new Set(watchlistRows.map((row) => String(row.band_status)))).sort().map((value) => <option key={value}>{value}</option>)}</select></label><label><span>Abstention</span><select aria-label="Watchlist abstention filter" value={abstentionFilter} onChange={(event) => setAbstentionFilter(event.target.value)}><option value="ALL">All states</option><option value="ABSTAIN">ABSTAIN</option><option value="NOT_ABSTAIN">NOT_ABSTAIN</option><option value="NOT_APPLICABLE">NOT APPLICABLE</option></select></label></div>
    <DenseTable rows={rows} columns={columns} searchPlaceholder="Search watchlist" />
  </div>;
}

function abstentionState(value: unknown): string {
  if (value === null || value === undefined) return "NOT_APPLICABLE";
  return Boolean(value) ? "ABSTAIN" : "NOT_ABSTAIN";
}
