...
export function CommandCenterPage({data}: {data: CommandCenterData}) {
  ...
  return <div className="page-stack">
    ...
    <div className="dashboard-grid two-one">
      ...
      <Panel title="Data / Provider Health"><dl className="metric-list"><div><dt>Operational primary</dt><dd>{String(providerHealth.operational_provider ?? "UNAVAILABLE")}</dd></div><div><dt>AKShare function</dt><dd>{String(providerHealth.operational_function ?? "UNAVAILABLE")}</dd></div><div><dt>Accepted / rejected</dt><dd>{String(providerHealth.accepted_symbol_count ?? "N/A")} / {String(providerHealth.rejected_symbol_count ?? "N/A")}</dd></div><div><dt>East Money canonical requests</dt><dd>{String(providerHealth.east_money_canonical_request_count ?? "UNAVAILABLE")}</dd></div><div><dt>Diagnostics</dt><dd>{providerComparisons}</dd></div><div><dt>Quarantined research rows</dt><dd>{quarantinedRows}</dd></div><div><dt>Adjustment convention</dt><dd>{String(providerHealth.adjustment_convention_status ?? "UNAVAILABLE")}</dd></div><div><dt>Amount contract</dt><dd>{String(providerHealth.amount_availability ?? "UNAVAILABLE")}</dd></div><div><dt>Silent averaging</dt><dd>{providerHealth.no_silent_averaging === true ? "DISABLED" : "UNKNOWN"}</dd></div></dl></Panel>
