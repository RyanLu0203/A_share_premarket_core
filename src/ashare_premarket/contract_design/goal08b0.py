...
def audit_goal08b0_recommendation_review_only_unlock_gate(root: Path) -> bool:
    ...
    if manifest.get("goal08b_target_status") != GOAL08B_ELIGIBLE_STATUS:
        failures.append("manifest_goal08b_target_not_future_review_only")
    ...
    if manifest.get("goal08b_implemented_in_repo") is not False:
        failures.append("manifest_goal08b_implemented_in_repo_not_false")
...
def evaluate_goal08b0_unlock_gate(bundle: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "goal08b_target_status": GOAL08B_LOCKED_STATUS if failures else GOAL08B_ELIGIBLE_STATUS,
        "goal08b_transition_rule": "locked_future_to_future_review_only_eligibility_only",
        "allowed_next_action": GOAL08B0_BLOCKED_NEXT if failures else GOAL08B0_ALLOWED_NEXT,
...
def _write_policy(root: Path, review: dict[str, object]) -> None:
    write_json(
        root / POLICY_PATH,
        {
            ...
            "goal08b_status_after_pass": GOAL08B_ELIGIBLE_STATUS,
            "goal08b_implemented_after_pass": False,
            "allowed_next_action_after_pass": GOAL08B0_ALLOWED_NEXT,
...
def _update_locked_capabilities(root: Path, review: dict[str, object]) -> None:
    ...
    payload[GOAL08B_WORKFLOW_ID] = GOAL08B_ELIGIBLE_STATUS if review["status"] != BLOCKED else False
    for key in [
        "position_band_recommendation",
        "signal_backtest",
        "portfolio_backtest",
...
def goal08b0_valid_unlock_evidence(root: Path) -> bool:
    ...
    return (
        ...
        and manifest.get("goal08b_target_status") == GOAL08B_ELIGIBLE_STATUS
        and manifest.get("goal08b_implemented_by_this_gate") is False
        and manifest.get("goal08b_implemented_in_repo") is False
