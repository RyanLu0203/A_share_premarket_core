...
def audit_goal090_position_band_review_only_unlock_gate(root: Path) -> bool:
    ...
    if manifest.get("goal09_target_status") != GOAL09_ELIGIBLE_STATUS:
        failures.append("manifest_goal09_target_not_future_review_only")
    ...
    if manifest.get("goal09_implemented_in_repo") is not False:
        failures.append("manifest_goal09_implemented_in_repo_not_false")
    ...
    if goal09.get("status") != GOAL09_ELIGIBLE_STATUS:
        failures.append("goal09_workflow_not_future_review_only")
    if goal09.get("implemented_in_repo") != "false":
        failures.append("goal09_workflow_marked_implemented")
...
def evaluate_goal090_unlock_gate(bundle: dict[str, object]) -> dict[str, object]:
    ...
    manifest = _manifest(status, goal08b_rows, warning_codes, sorted(set(failures)))
    return {
        "status": status,
...
def goal090_valid_unlock_evidence(root: Path) -> bool:
    ...
    return (
        ...
        and manifest.get("goal09_target_status") == GOAL09_ELIGIBLE_STATUS
        and manifest.get("goal09_implemented_by_this_gate") is False
        and manifest.get("goal09_implemented_in_repo") is False
...
def _validate_input_bundle(bundle: dict[str, object]) -> list[str]:
    ...
    goal09 = workflow.get(GOAL09_WORKFLOW_ID, {})
    if goal09 and goal09.get("status") not in {"locked_future", GOAL09_ELIGIBLE_STATUS}:
        failures.append("goal09_workflow_not_locked_or_future_review_only")
    if goal09 and goal09.get("implemented_in_repo") != "false":
        failures.append("goal09_workflow_marked_implemented_before_gate")
    for workflow_id in DOWNSTREAM_LOCKED_IDS:
        row = workflow.get(workflow_id, {})
        if row.get("status") != "locked_future":
            failures.append(f"{workflow_id}_not_locked_future")
...
def _manifest(
    ...
    return {
        ...
        "goal09_target_status": "locked_future" if status == BLOCKED else GOAL09_ELIGIBLE_STATUS,
        "goal09_implemented_by_this_gate": False,
        "goal09_implemented_in_repo": False,
        "goal09_status_after_goal090": "locked_future" if status == BLOCKED else GOAL09_ELIGIBLE_STATUS,
        "allowed_next_action": GOAL090_BLOCKED_NEXT if status == BLOCKED else GOAL09_ALLOWED_NEXT,
...
def _write_policy(root: Path, review: dict[str, object]) -> None:
    write_json(
        root / POLICY_PATH,
        {
            ...
            "target_status_after_pass": GOAL09_ELIGIBLE_STATUS,
            "goal09_implemented_by_this_gate": False,
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                "",
                "GOAL-09.0 is an unlock-only governance gate. It may mark GOAL-09 position-band diagnostics as `future_review_only` eligible for a later explicit non-actionable prototype request. It does not implement GOAL-09.",
                "",
                "## Evidence Basis",
                "",
                ...
                "## Boundary",
                "",
                "- GOAL-09 remains not implemented.",
                "- Future position-band diagnostics require a separate explicit request.",
...
def _update_workflow_status(root: Path, review: dict[str, object]) -> None:
    ...
    if GOAL09_WORKFLOW_ID in by_id:
        by_id[GOAL09_WORKFLOW_ID].update(
            {
                "display_name": "GOAL-09 Position-Band Diagnostics Prototype",
                "stage_or_goal": "GOAL-09",
                "status": GOAL09_ELIGIBLE_STATUS if review["status"] != BLOCKED else "locked_future",
                "current_repo_role": "review_only_eligible_not_implemented" if review["status"] != BLOCKED else "locked_downstream_boundary",
                "implemented_in_repo": "false",
                "allowed_next_action": GOAL09_ALLOWED_NEXT if review["status"] != BLOCKED else "remain_locked_until_goal090_passes",
...
def _update_locked_capabilities(root: Path, review: dict[str, object]) -> None:
    ...
    payload[GOAL09_WORKFLOW_ID] = GOAL09_ELIGIBLE_STATUS if review["status"] != BLOCKED else False
    for key in [
        "signal_backtest",
        "portfolio_backtest",
        "dashboard",
...
def goal09_eligible_workflow_patch() -> dict[str, str]:
    return {
        "display_name": "GOAL-09 Position-Band Diagnostics Prototype",
        "stage_or_goal": "GOAL-09",
