...
def run_workflow_diagnostics(root: Path) -> bool:
    ...
    goal_quant_research01_status = _goal_quant_research01_status(root)
    goal_quant_research01_manifest = _goal_quant_research01_manifest(root)
    goal_quant_research01_audit_status = _audit_status(root / "outputs/audits/goal_quant_research01_factor_research_lab_audit.md")
    goal_mvp01_status = _goal_mvp01_status(root)
    goal_mvp01_manifest = _goal_mvp01_manifest(root)
    goal_mvp01_audit_status = _audit_status(root / "outputs/audits/goal_mvp01_premarket_terminal_audit.md")
    ...
    goal_alpha_research_refinement01_status = _goal_alpha_research_refinement01_status(root)
    goal_alpha_research_refinement01_manifest = _goal_alpha_research_refinement01_manifest(root)
    goal_alpha_research_refinement01_audit_status = _audit_status(root / "outputs/audits/goal_alpha_research_refinement01_audit.md")
    goal_alpha_factor_candidate02_status = _goal_alpha_factor_candidate02_status(root)
    goal_alpha_factor_candidate02_manifest = _goal_alpha_factor_candidate02_manifest(root)
    goal_alpha_factor_candidate02_audit_status = _audit_status(root / "outputs/audits/goal_alpha_factor_candidate02_audit.md")
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                f"GOAL-RISK-TIERING-01 risk tiering status: `{goal_risk_tiering01_status}`.",
                f"GOAL-RISK-TIERING-01 audit status: `{goal_risk_tiering01_audit_status}`.",
                ...
                f"GOAL-QUANT-RESEARCH-01 factor lab status: `{goal_quant_research01_status}`.",
                f"GOAL-QUANT-RESEARCH-01 audit status: `{goal_quant_research01_audit_status}`.",
                f"GOAL-QUANT-RESEARCH-01 factors evaluated: `{goal_quant_research01_manifest.get('factor_count', 0)}`.",
                f"GOAL-QUANT-RESEARCH-01 evaluation rows: `{goal_quant_research01_manifest.get('factor_evaluation_row_count', 0)}`.",
                f"GOAL-QUANT-RESEARCH-01 ready factor count: `{goal_quant_research01_manifest.get('ready_factor_count', 0)}`.",
                f"GOAL-QUANT-RESEARCH-01 score validity: `{goal_quant_research01_manifest.get('overall_score_validity_status', 'not yet generated')}`.",
                f"GOAL-MVP-01 premarket terminal status: `{goal_mvp01_status}`.",
                f"GOAL-MVP-01 audit status: `{goal_mvp01_audit_status}`.",
                f"GOAL-MVP-01 report date: `{goal_mvp01_manifest.get('report_date', 'not yet generated')}`.",
                f"GOAL-MVP-01 symbol rows: `{goal_mvp01_manifest.get('symbol_table_row_count', 0)}`.",
                f"GOAL-MVP-01 ready factor count: `{goal_mvp01_manifest.get('ready_factor_count', 0)}`.",
                f"GOAL-ALPHA-FACTOR-CANDIDATE-01 status: `{goal_alpha_factor_candidate01_status}`.",
                ...
                f"GOAL-ALPHA-RESEARCH-REFINEMENT-01 status: `{goal_alpha_research_refinement01_status}`.",
                f"GOAL-ALPHA-RESEARCH-REFINEMENT-01 audit status: `{goal_alpha_research_refinement01_audit_status}`.",
                f"GOAL-ALPHA-RESEARCH-REFINEMENT-01 promising candidates diagnosed: `{goal_alpha_research_refinement01_manifest.get('promising_candidate_count', 0)}`.",
                f"GOAL-ALPHA-RESEARCH-REFINEMENT-01 refined design rows: `{goal_alpha_research_refinement01_manifest.get('refined_candidate_design_count', 0)}`.",
                f"GOAL-ALPHA-RESEARCH-REFINEMENT-01 trial registry rows: `{goal_alpha_research_refinement01_manifest.get('trial_registry_update_row_count', 0)}`.",
                f"GOAL-ALPHA-FACTOR-CANDIDATE-02 refined candidate construction status: `{goal_alpha_factor_candidate02_status}`.",
                f"GOAL-ALPHA-FACTOR-CANDIDATE-02 audit status: `{goal_alpha_factor_candidate02_audit_status}`.",
                f"GOAL-ALPHA-FACTOR-CANDIDATE-02 constructed refined candidate count: `{goal_alpha_factor_candidate02_manifest.get('constructed_refined_candidate_count', 0)}`.",
                f"GOAL-ALPHA-FACTOR-CANDIDATE-02 refined candidate panel rows: `{goal_alpha_factor_candidate02_manifest.get('refined_candidate_panel_row_count', 0)}`.",
                f"GOAL-ALPHA-FACTOR-CANDIDATE-02 trial registry rows: `{goal_alpha_factor_candidate02_manifest.get('trial_registry_row_count', 0)}`.",
                ...
                f"GOAL-ALPHA-RESEARCH-REFINEMENT-01 workflow status: `{downstream_status.get('goal_alpha_research_refinement01_rolling_stability_candidate_refinement_gate', 'missing')}`.",
                f"GOAL-ALPHA-FACTOR-CANDIDATE-02 workflow status: `{downstream_status.get('goal_alpha_factor_candidate02_refined_variants_research_gate', 'missing')}`.",
                f"GOAL-QUANT-RESEARCH-03 workflow status: `{downstream_status.get('goal_quant_research03_refined_alpha_factor_validity_evaluation_gate', 'missing')}`.",
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                "40. For GOAL-ALPHA-RESEARCH-REFINEMENT-01, run `python scripts/run_goal_alpha_research_refinement01_gate.py` and `python scripts/audit_goal_alpha_research_refinement01_gate.py`; it may create only rolling-stability attribution, conditional stability summaries, refined candidate design definitions, intraday redefinition plans, and trial-registry updates.",
                "41. For GOAL-ALPHA-FACTOR-CANDIDATE-02, run `python scripts/run_goal_alpha_factor_candidate02_gate.py` and `python scripts/audit_goal_alpha_factor_candidate02_gate.py`; it may create only refined candidate factor values and must not evaluate predictive validity, run GOAL-QUANT-RESEARCH-03, or create recommendation, position, portfolio, dashboard, trading, production, local-lake, factor-mining, broker, or DQN/RL outputs.",
                "42. GOAL-QUANT-RESEARCH-03 remains locked until an explicit refined-factor validity evaluation request.",
                "43. V2 factor research is planned but inactive; do not create factor mining, IC/RankIC mining, factor libraries, or factor outputs in V1.",
                "44. Do not unlock recommendation execution, actual positions, position sizing, dashboard, paper/live trading, production writes, model promotion, portfolio backtests, factor mining, broker, local-lake, or DQN/RL.",
                "",
            ]
        ),
    )
...
def _goal_risk_tiering01_status(root: Path) -> str:
    report = root / "outputs/audits/goal_risk_tiering01_risk_tiering_report.md"
    if not report.exists():
        return "not yet generated"
    text = report.read_text(encoding="utf-8")
    if "GOAL-RISK-TIERING-01 Risk Severity Numeric Score Tiering Gate: BLOCKED" in text:
        return "BLOCKED"
    if "GOAL-RISK-TIERING-01 Risk Severity Numeric Score Tiering Gate: PASS_WITH_WARNINGS" in text:
        return "PASS_WITH_WARNINGS"
    if "GOAL-RISK-TIERING-01 Risk Severity Numeric Score Tiering Gate: PASS" in text:
        return "PASS"
    return "unknown"


def _goal_risk_tiering01_manifest(root: Path) -> dict[str, object]:
    path = root / "outputs/audits/goal_risk_tiering01_risk_tiering_manifest.json"
    if not path.exists():
        return {}
    try:
        return read_json(path)
...
def _goal_quant_research01_status(root: Path) -> str:
    report = root / "outputs/audits/goal_quant_research01_factor_research_lab_report.md"
    if not report.exists():
        return "not yet generated"
    text = report.read_text(encoding="utf-8")
    if "GOAL-QUANT-RESEARCH-01 Factor Research Lab: BLOCKED" in text:
        return "BLOCKED"
    if "GOAL-QUANT-RESEARCH-01 Factor Research Lab: PASS_WITH_WARNINGS" in text:
        return "PASS_WITH_WARNINGS"
    if "GOAL-QUANT-RESEARCH-01 Factor Research Lab: PASS" in text:
        return "PASS"
    return "unknown"


def _goal_quant_research01_manifest(root: Path) -> dict[str, object]:
    path = root / "outputs/audits/goal_quant_research01_factor_research_lab_manifest.json"
    if not path.exists():
        return {}
    try:
        return read_json(path)
...
def _goal_mvp01_status(root: Path) -> str:
    report = root / "outputs/audits/goal_mvp01_premarket_terminal_report.md"
    if not report.exists():
        return "not yet generated"
    text = report.read_text(encoding="utf-8")
    if "GOAL-MVP-01 Premarket Research Diagnostic Terminal Gate: BLOCKED" in text:
        return "BLOCKED"
    if "GOAL-MVP-01 Premarket Research Diagnostic Terminal Gate: PASS_WITH_WARNINGS" in text:
        return "PASS_WITH_WARNINGS"
    if "GOAL-MVP-01 Premarket Research Diagnostic Terminal Gate: PASS" in text:
        return "PASS"
    return "unknown"


def _goal_mvp01_manifest(root: Path) -> dict[str, object]:
    path = root / "outputs/audits/goal_mvp01_premarket_terminal_manifest.json"
    if not path.exists():
        return {}
    try:
        return read_json(path)
...
def _goal_alpha_factor_candidate01_status(root: Path) -> str:
    report = root / "outputs/audits/goal_alpha_factor_candidate01_report.md"
    if not report.exists():
        return _workflow_status(root, "goal_alpha_factor_candidate01_research_gate")
    text = report.read_text(encoding="utf-8")
    if "GOAL-ALPHA-FACTOR-CANDIDATE-01 Research Grade Alpha Candidate Construction Gate: BLOCKED" in text:
        return "BLOCKED"
    if "GOAL-ALPHA-FACTOR-CANDIDATE-01 Research Grade Alpha Candidate Construction Gate: PASS_WITH_WARNINGS" in text:
        return "PASS_WITH_WARNINGS"
    if "GOAL-ALPHA-FACTOR-CANDIDATE-01 Research Grade Alpha Candidate Construction Gate: PASS" in text:
        return "PASS"
    return _workflow_status(root, "goal_alpha_factor_candidate01_research_gate")


def _goal_alpha_factor_candidate01_manifest(root: Path) -> dict[str, object]:
    path = root / "outputs/audits/goal_alpha_factor_candidate01_manifest.json"
    if not path.exists():
        return {}
    try:
        return read_json(path)
...
def _goal_alpha_research_refinement01_status(root: Path) -> str:
    report = root / "outputs/audits/goal_alpha_research_refinement01_report.md"
    if not report.exists():
        return _workflow_status(root, "goal_alpha_research_refinement01_rolling_stability_candidate_refinement_gate")
    text = report.read_text(encoding="utf-8")
    if "GOAL-ALPHA-RESEARCH-REFINEMENT-01 Rolling Stability and Candidate Refinement Gate: BLOCKED" in text:
        return "BLOCKED"
    if "GOAL-ALPHA-RESEARCH-REFINEMENT-01 Rolling Stability and Candidate Refinement Gate: PASS_WITH_WARNINGS" in text:
        return "PASS_WITH_WARNINGS"
    if "GOAL-ALPHA-RESEARCH-REFINEMENT-01 Rolling Stability and Candidate Refinement Gate: PASS" in text:
        return "PASS"
    return _workflow_status(root, "goal_alpha_research_refinement01_rolling_stability_candidate_refinement_gate")


def _goal_alpha_research_refinement01_manifest(root: Path) -> dict[str, object]:
    path = root / "outputs/audits/goal_alpha_research_refinement01_manifest.json"
    if not path.exists():
        return {}
    try:
        return read_json(path)
...
def _goal_alpha_factor_candidate02_status(root: Path) -> str:
    report = root / "outputs/audits/goal_alpha_factor_candidate02_report.md"
    if not report.exists():
        return _workflow_status(root, "goal_alpha_factor_candidate02_refined_variants_research_gate")
    text = report.read_text(encoding="utf-8")
    if "GOAL-ALPHA-FACTOR-CANDIDATE-02 Refined Alpha Candidate Construction Gate: BLOCKED" in text:
        return "BLOCKED"
    if "GOAL-ALPHA-FACTOR-CANDIDATE-02 Refined Alpha Candidate Construction Gate: PASS_WITH_WARNINGS" in text:
        return "PASS_WITH_WARNINGS"
    if "GOAL-ALPHA-FACTOR-CANDIDATE-02 Refined Alpha Candidate Construction Gate: PASS" in text:
        return "PASS"
    return _workflow_status(root, "goal_alpha_factor_candidate02_refined_variants_research_gate")


def _goal_alpha_factor_candidate02_manifest(root: Path) -> dict[str, object]:
    path = root / "outputs/audits/goal_alpha_factor_candidate02_manifest.json"
    if not path.exists():
        return {}
    try:
        return read_json(path)
...
def _downstream_lock_status(root: Path) -> dict[str, str]:
    ...
    return {
        ...
        for key in [
            ...
            "goal_alpha_research_refinement01_rolling_stability_candidate_refinement_gate",
