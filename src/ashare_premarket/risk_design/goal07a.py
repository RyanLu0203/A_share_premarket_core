...
def _audit_boundary_locks(root: Path) -> str:
    ...
    for workflow_id in DOWNSTREAM_LOCKED_IDS:
        ...
        if workflow_id == GOAL09_WORKFLOW_ID and goal090_valid:
            goal09_expected = goal09_eligible_workflow_patch(root)
            downstream_locked = downstream_locked and row.get("status") == goal09_expected.get("status") and row.get("implemented_in_repo") == goal09_expected.get("implemented_in_repo")
        else:
            downstream_locked = downstream_locked and row.get("status") == "locked_future"
