...
def preserve_later_review_only_workflow_states(root: Path, by_id: dict[str, dict[str, str]]) -> None:
    ...
    if _goal_quant_research01_valid(root):
        from ashare_premarket.research.goal_quant_research01 import (
            ...
            GOAL_REC_TIERING01_WORKFLOW_ID,
            POSITION_BAND_VALIDATION_WORKFLOW_ID,
            WORKFLOW_ID as GOAL_QUANT_RESEARCH01_WORKFLOW_ID,
            goal_quant_research01_implemented_workflow_patch,
            locked_goal10b4_patch,
            locked_goal10d_patch as locked_goal10d_after_goal_quant_research01_patch,
            locked_goal_rec_tiering01_patch,
            locked_position_band_validation_patch,
        )

        if GOAL_QUANT_RESEARCH01_WORKFLOW_ID in by_id:
            by_id[GOAL_QUANT_RESEARCH01_WORKFLOW_ID].update(goal_quant_research01_implemented_workflow_patch())
        if GOAL_REC_TIERING01_WORKFLOW_ID in by_id:
            by_id[GOAL_REC_TIERING01_WORKFLOW_ID].update(locked_goal_rec_tiering01_patch())
        if GOAL10B4_WORKFLOW_ID in by_id:
            by_id[GOAL10B4_WORKFLOW_ID].update(locked_goal10b4_patch())
        if POSITION_BAND_VALIDATION_WORKFLOW_ID in by_id:
            by_id[POSITION_BAND_VALIDATION_WORKFLOW_ID].update(locked_position_band_validation_patch())
        if GOAL10D_WORKFLOW_ID in by_id:
            by_id[GOAL10D_WORKFLOW_ID].update(locked_goal10d_after_goal_quant_research01_patch())
        for workflow_id in [
            "dashboard_daily_report",
            "signal_backtest",
            "portfolio_backtest",
    ...
    if _goal_mvp01_valid(root):
        ...
        if (
            GOAL_ALPHA_FACTOR_CANDIDATE01_WORKFLOW_ID in by_id
            and by_id[GOAL_ALPHA_FACTOR_CANDIDATE01_WORKFLOW_ID].get("status") != "implemented_research_only"
        ):
    ...
    if _goal_alpha_factor_candidate01_valid(root):
        from ashare_premarket.research.goal_alpha_factor_candidate01 import (
            ...
            GOAL_REC_TIERING01_WORKFLOW_ID,
            POSITION_BAND_VALIDATION_WORKFLOW_ID,
            WORKFLOW_ID as GOAL_ALPHA_FACTOR_CANDIDATE01_WORKFLOW_ID,
            goal_alpha_factor_candidate01_implemented_workflow_patch,
            locked_goal10b4_patch,
            locked_goal10d_patch,
            locked_goal_quant_research02_patch,
            locked_goal_rec_tiering01_patch,
            locked_position_band_validation_patch,
        )

        if GOAL_ALPHA_FACTOR_CANDIDATE01_WORKFLOW_ID in by_id:
            by_id[GOAL_ALPHA_FACTOR_CANDIDATE01_WORKFLOW_ID].update(
                goal_alpha_factor_candidate01_implemented_workflow_patch()
            )
        if (
            GOAL_QUANT_RESEARCH02_WORKFLOW_ID in by_id
            and by_id[GOAL_QUANT_RESEARCH02_WORKFLOW_ID].get("status") != "implemented_research_only"
        ):
    ...
    if _goal_quant_research02_valid(root):
        from ashare_premarket.research.goal_quant_research02 import (
            GOAL10B4_WORKFLOW_ID,
            GOAL10D_WORKFLOW_ID,
            GOAL_REC_TIERING01_WORKFLOW_ID,
            POSITION_BAND_VALIDATION_WORKFLOW_ID,
            WORKFLOW_ID as GOAL_QUANT_RESEARCH02_WORKFLOW_ID,
            goal_quant_research02_implemented_workflow_patch,
            locked_goal10b4_patch,
            locked_goal10d_patch,
            locked_goal_rec_tiering01_patch,
            locked_position_band_validation_patch,
        ...
        if GOAL_QUANT_RESEARCH02_WORKFLOW_ID in by_id:
            by_id[GOAL_QUANT_RESEARCH02_WORKFLOW_ID].update(goal_quant_research02_implemented_workflow_patch())
        if GOAL_REC_TIERING01_WORKFLOW_ID in by_id:
            by_id[GOAL_REC_TIERING01_WORKFLOW_ID].update(locked_goal_rec_tiering01_patch())
        if GOAL10B4_WORKFLOW_ID in by_id:
            by_id[GOAL10B4_WORKFLOW_ID].update(locked_goal10b4_patch())
    ...
    if _goal_alpha_research_refinement01_valid(root):
        from ashare_premarket.research.goal_alpha_research_refinement01 import (
            ...
            GOAL_REC_TIERING01_WORKFLOW_ID,
            POSITION_BAND_VALIDATION_WORKFLOW_ID,
            WORKFLOW_ID as GOAL_ALPHA_RESEARCH_REFINEMENT01_WORKFLOW_ID,
            goal_alpha_research_refinement01_implemented_workflow_patch,
            locked_goal10b4_patch,
            locked_goal10d_patch,
            locked_goal_alpha_factor_candidate02_patch,
            locked_goal_rec_tiering01_patch,
            locked_position_band_validation_patch,
        )

        if GOAL_ALPHA_RESEARCH_REFINEMENT01_WORKFLOW_ID in by_id:
            by_id[GOAL_ALPHA_RESEARCH_REFINEMENT01_WORKFLOW_ID].update(
                goal_alpha_research_refinement01_implemented_workflow_patch()
            )
        goal_alpha_factor_candidate02_already_implemented = (
            GOAL_ALPHA_FACTOR_CANDIDATE02_WORKFLOW_ID in by_id
            and by_id[GOAL_ALPHA_FACTOR_CANDIDATE02_WORKFLOW_ID].get("status") == "implemented_research_only"
        )
        if GOAL_ALPHA_FACTOR_CANDIDATE02_WORKFLOW_ID in by_id and not goal_alpha_factor_candidate02_already_implemented:
            by_id[GOAL_ALPHA_FACTOR_CANDIDATE02_WORKFLOW_ID].update(locked_goal_alpha_factor_candidate02_patch())
        if GOAL_REC_TIERING01_WORKFLOW_ID in by_id and not goal_alpha_factor_candidate02_already_implemented:
            by_id[GOAL_REC_TIERING01_WORKFLOW_ID].update(locked_goal_rec_tiering01_patch())
        if GOAL10B4_WORKFLOW_ID in by_id and not goal_alpha_factor_candidate02_already_implemented:
            by_id[GOAL10B4_WORKFLOW_ID].update(locked_goal10b4_patch())
        if POSITION_BAND_VALIDATION_WORKFLOW_ID in by_id and not goal_alpha_factor_candidate02_already_implemented:
            by_id[POSITION_BAND_VALIDATION_WORKFLOW_ID].update(locked_position_band_validation_patch())
        if GOAL10D_WORKFLOW_ID in by_id and not goal_alpha_factor_candidate02_already_implemented:
            by_id[GOAL10D_WORKFLOW_ID].update(locked_goal10d_patch())
        for workflow_id in [
            "dashboard_daily_report",
            "signal_backtest",
    ...
    if _goal_alpha_factor_candidate02_valid(root):
        from ashare_premarket.research.goal_alpha_factor_candidate02 import (
            GOAL10B4_WORKFLOW_ID,
            GOAL10D_WORKFLOW_ID,
            GOAL_QUANT_RESEARCH03_WORKFLOW_ID,
            GOAL_REC_TIERING01_WORKFLOW_ID,
            POSITION_BAND_VALIDATION_WORKFLOW_ID,
            WORKFLOW_ID as GOAL_ALPHA_FACTOR_CANDIDATE02_WORKFLOW_ID,
            goal_alpha_factor_candidate02_implemented_workflow_patch,
            locked_goal10b4_patch,
            locked_goal10d_patch,
            locked_goal_quant_research03_patch,
            locked_goal_rec_tiering01_patch,
            locked_position_band_validation_patch,
        )

        if GOAL_ALPHA_FACTOR_CANDIDATE02_WORKFLOW_ID in by_id:
            by_id[GOAL_ALPHA_FACTOR_CANDIDATE02_WORKFLOW_ID].update(
                goal_alpha_factor_candidate02_implemented_workflow_patch()
            )
    ...
    if _goal_quant_research03_valid(root):
        from ashare_premarket.research.goal_quant_research03 import (
            GOAL10B4_WORKFLOW_ID,
            GOAL10D_WORKFLOW_ID,
            GOAL_REC_TIERING01_WORKFLOW_ID,
            POSITION_BAND_VALIDATION_WORKFLOW_ID,
            WORKFLOW_ID as GOAL_QUANT_RESEARCH03_WORKFLOW_ID,
            goal_quant_research03_implemented_workflow_patch,
            locked_goal10b4_patch,
            locked_goal10d_patch,
            locked_goal_rec_tiering01_patch,
            locked_position_band_validation_patch,
        ...
        if GOAL_QUANT_RESEARCH03_WORKFLOW_ID in by_id:
            by_id[GOAL_QUANT_RESEARCH03_WORKFLOW_ID].update(
                goal_quant_research03_implemented_workflow_patch()
            )
        if GOAL_REC_TIERING01_WORKFLOW_ID in by_id:
            by_id[GOAL_REC_TIERING01_WORKFLOW_ID].update(locked_goal_rec_tiering01_patch())
    ...
    if _goal_regime_label_research01_valid(root):
        from ashare_premarket.research.goal_regime_label_research01 import (
            ...
            GOAL_REC_TIERING01_WORKFLOW_ID,
            POSITION_BAND_VALIDATION_WORKFLOW_ID,
            WORKFLOW_ID as GOAL_REGIME_LABEL_RESEARCH01_WORKFLOW_ID,
            goal_regime_label_research01_implemented_workflow_patch,
            locked_goal10b4_patch,
            locked_goal10d_patch,
        ...
        if GOAL_REGIME_LABEL_RESEARCH01_WORKFLOW_ID in by_id:
            by_id[GOAL_REGIME_LABEL_RESEARCH01_WORKFLOW_ID].update(
                goal_regime_label_research01_implemented_workflow_patch()
            )
...
def preserve_later_review_only_capabilities(root: Path, payload: dict[str, object]) -> None:
    ...
    if _goal_mvp01_valid(root):
        ...
        payload["goal10d_backtest_failure_attribution_gate"] = False
    if _goal_alpha_factor_candidate01_valid(root):
        payload["goal_alpha_factor_candidate01_research_gate"] = "implemented_research_only"
        payload["goal_quant_research02_alpha_candidate_factor_validity_evaluation_gate"] = False
        payload["goal_rec_tiering01_recommendation_score_tiering_gate"] = False
        payload["goal10b4_recommendation_backtest_revalidation"] = False
        payload["goal_position_band_validation01_position_band_validation_gate"] = False
        payload["goal10d_backtest_failure_attribution_gate"] = False
    if _goal_quant_research02_valid(root):
        payload["goal_quant_research02_alpha_candidate_factor_validity_evaluation_gate"] = "implemented_research_only"
        payload["goal_rec_tiering01_recommendation_score_tiering_gate"] = False
        payload["goal10b4_recommendation_backtest_revalidation"] = False
        payload["goal_position_band_validation01_position_band_validation_gate"] = False
        payload["goal10d_backtest_failure_attribution_gate"] = False
    if _goal_alpha_research_refinement01_valid(root):
        payload["goal_alpha_research_refinement01_rolling_stability_candidate_refinement_gate"] = "implemented_research_only"
        if not _goal_alpha_factor_candidate02_valid(root):
            payload["goal_alpha_factor_candidate02_refined_variants_research_gate"] = False
        payload["goal_rec_tiering01_recommendation_score_tiering_gate"] = False
        payload["goal10b4_recommendation_backtest_revalidation"] = False
        payload["goal_position_band_validation01_position_band_validation_gate"] = False
        payload["goal10d_backtest_failure_attribution_gate"] = False
    if _goal_alpha_factor_candidate02_valid(root):
        payload["goal_alpha_factor_candidate02_refined_variants_research_gate"] = "implemented_research_only"
        payload["goal_quant_research03_refined_alpha_factor_validity_evaluation_gate"] = False
        payload["goal_rec_tiering01_recommendation_score_tiering_gate"] = False
        payload["goal10b4_recommendation_backtest_revalidation"] = False
        payload["goal_position_band_validation01_position_band_validation_gate"] = False
        payload["goal10d_backtest_failure_attribution_gate"] = False
    if _goal_quant_research03_valid(root):
        payload["goal_quant_research03_refined_alpha_factor_validity_evaluation_gate"] = "implemented_research_only"
        payload["goal_rec_tiering01_recommendation_score_tiering_gate"] = False
        payload["goal10b4_recommendation_backtest_revalidation"] = False
        payload["goal_position_band_validation01_position_band_validation_gate"] = False
        payload["goal10d_backtest_failure_attribution_gate"] = False
    if _goal_regime_label_research01_valid(root):
        ...
        payload["goal_quant_research04_regime_conditional_factor_evaluation_gate"] = "implemented_research_only" if _goal_quant_research04_valid(root) else False
        payload["goal_rec_tiering01_recommendation_score_tiering_gate"] = False
        payload["goal10b4_recommendation_backtest_revalidation"] = False
        payload["goal_position_band_validation01_position_band_validation_gate"] = False
        payload["goal10d_backtest_failure_attribution_gate"] = False
...
def _goal_alpha_factor_candidate01_valid(root: Path) -> bool:
    try:
        from ashare_premarket.research.goal_alpha_factor_candidate01 import goal_alpha_factor_candidate01_valid_evidence

        return goal_alpha_factor_candidate01_valid_evidence(root)
    except Exception:
        return False
...
def _goal_alpha_research_refinement01_valid(root: Path) -> bool:
    try:
        from ashare_premarket.research.goal_alpha_research_refinement01 import (
            goal_alpha_research_refinement01_valid_evidence,
        )

        return goal_alpha_research_refinement01_valid_evidence(root)
    except Exception:
        return False


def _goal_alpha_factor_candidate02_valid(root: Path) -> bool:
    try:
        from ashare_premarket.research.goal_alpha_factor_candidate02 import goal_alpha_factor_candidate02_valid_evidence

        return goal_alpha_factor_candidate02_valid_evidence(root)
    except Exception:
        return False
...
def _goal_regime_label_research01_valid(root: Path) -> bool:
    try:
