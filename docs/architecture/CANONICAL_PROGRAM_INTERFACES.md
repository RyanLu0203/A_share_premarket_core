...
authoritative branch is `project-current`; `main` is not a deployment source.
...
| Workspace frontend | Present read-only evidence | `npm.cmd run dev --prefix apps/premarket-workspace` | `apps/premarket-workspace` | `NEXT_PUBLIC_PREMARKET_API_URL` | Next.js workspace | Configured loopback API only | Shared live/replay and snapshot parameters | Fails on missing dependencies/API | Public |
