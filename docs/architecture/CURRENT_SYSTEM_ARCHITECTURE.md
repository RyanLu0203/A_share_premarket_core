...
| iFinD AI 金融数据服务 | S0 accepted; S1 repair pending live acceptance | The MCP/API Key channel passed governed S0 for seven services and 35/35 personal/trial entitled tools plus live schemas. Two bounded runs stopped after their first Luxshare call. The complete portal response confirmed an exact supplier code/name column inversion; a strict one-row allowlisted identity repair is implemented offline but not yet live-accepted. Hengtong was not called and no live iFinD row is accepted. S2-S4 and research remain locked. |
