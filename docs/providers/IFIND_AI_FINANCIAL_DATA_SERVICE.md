...
> `edb:search_edb` 标记为 `UNAVAILABLE_BY_PLAN`。第一次 S1 立讯响应在证券代码
> 范围校验处隔离；精确代码映射合并后，第二次授权运行在首个立讯调用的响应结构
> 校验处停止。完整 portal 响应随后确认供应商把 `证券代码` 与 `证券简称` 的行值
> 反放；严格的单行、精确证券代码与公司名修复已离线实现。亨通光电尚未调用、
> 前两次均未重试、无数据接纳。合并后的第三次运行仍在首个立讯调用停止；离线
> 对照确认客户端把估值/数据时点混入了仅做身份验收的 S1 请求。查询现已收窄为
> 精确公司简称，仍待合并、部署和单独授权的 live S1 复验。
...
企业版专属。第一次 S1 立讯摘要因范围未验证被隔离；精确代码映射合并后，第二次
授权运行在首个立讯调用以 `IFIND_MCP_RESPONSE_SCHEMA_MISMATCH` 停止。亨通尚未
调用，两次均未重试，仍未接纳任何 iFinD 数据。完整 portal 响应已确认代码/简称
列值反放，严格身份修复已合并。第三次运行仍在立讯处停止，原因定位为客户端
查询超出身份验收范围；`get_stock_summary` 查询已收窄为精确公司简称。再次 live
S1 仍需合并、部署和单独授权。
...
- 保持旧暴露凭据永久禁用；记录用户已报告完成轮换，同时将外部认证状态保持
  为 `PENDING_EXTERNAL_AUTH_ACCEPTANCE`；
- 默认离线探针通过；
