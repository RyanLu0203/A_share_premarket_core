...
The consolidation is complete: `<private-macos-home>/Desktop/A_share_premarket_core`
...
| --- | --- |
| `<private-macos-home>/Desktop/A_share_premarket_core` | Only top-level active development checkout |
...
| `<private-macos-home>/Library/Application Support/AsharePremarket/deployment` | Stable deployment root; unchanged and still used by loaded services |
...
| --- | ---: | --- | --- |
| `<private-macos-home>/Desktop/A_share_premarket_core` | 2.0 GiB | active feature branch; expected current changes | Active development checkout for the present work |
| `<private-macos-home>/Library/Application Support/AsharePremarket/deployment` | 1.7 GiB | clean `project-current` at `25273bb` | Stable non-TCC deployment root used by the loaded macOS services |
| `<private-macos-home>/Desktop/A_share_premarket_core_checkpoint_310559.bundle` | about 16 MiB | private Git bundle, outside the repository | User-private rollback evidence; not a runtime or Codex Max dependency |
...
| --- | ---: | --- | --- |
| `<private-macos-home>/Desktop/A_share_premarket_core_current` | 1.9 GiB | clean Git checkout | Confirm no shell, editor, service, or worktree depends on it; record its exact commit and remote |
| `<private-macos-home>/Desktop/A_share_premarket_core_deployment_repair` | 1.2 GiB | clean linked worktree at `be22ea4` | Remove through Git worktree governance, never by blind recursive deletion |
| `<private-macos-home>/Desktop/A_share_premarket_core_current.rollback-pre-pr32-20260714` | 1.6 GiB | clean rollback checkout | Confirm the private bundle and remote refs provide sufficient rollback coverage |
| `<private-macos-home>/Desktop/A_share_premarket_core_current.rollback-evidence-pre-pr38-20260717T1100` | 12 MiB | non-Git rollback evidence | Inspect contents and retention need before any move or deletion |
...
| --- | ---: | --- | --- |
| `<private-macos-home>/Desktop/A_share_premarket_core_deploy` | 1.8 GiB | about 100 status entries on `codex/local-mac-deployment` | May contain uncommitted deployment/runtime evidence |
| `<private-macos-home>/Desktop/A_share_premarket_core_runtime` | 1.8 GiB | about 98 status entries on `project-current` | May contain local runtime data and user changes |
| `<private-macos-home>/Desktop/A_share_premarket_core_ops` | 1.8 GiB | dirty and four commits ahead of its recorded `origin/project-current` | Contains branch history and at least one working-tree change |
